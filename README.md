...
Make your code reliable, readable, and secure. Essential techniques include using strict error handling with `set -euo pipefail`, always quoting variable expansions, and modularizing logic with functions.

### 1. Strict Error Handling: `set -euo pipefail`

When you write a standard Bash script, its default behavior is incredibly forgiving. If a command fails, Bash shrugs its shoulders and moves on to the next line. If you misspell a variable, Bash treats it as an empty string.

While this "keep calm and carry on" attitude sounds nice, it is a recipe for disaster in production scripts. It can lead to silent data corruption, infinite loops, or accidental file deletion.

The command `set -euo pipefail` is often called the "Unofficial Bash Strict Mode." It forces Bash to behave more like a modern programming language by crashing immediately when something goes wrong.

Here is a detailed breakdown of exactly what each part does:

#### `set -e` (errexit)
By default, Bash only cares about the very last line of a script. If lines 1 through 99 fail catastrophically, but line 100 succeeds, the script exits with a success code (`0`).

`set -e` changes this. It tells Bash: "If any command returns a non-zero exit status (an error), abort the script immediately."

**The Danger Without It:**
```bash
#!/bin/bash
cd /var/www/my-important-project
rm -rf *
```
*If the directory `/var/www/my-important-project` doesn't exist or you don't have permission to access it, the `cd` command fails. But without `set -e`, Bash blindly executes the next line: `rm -rf *`. Since you didn't change directories, it will delete everything in whatever directory you ran the script from.*

**With `set -e`:**
The moment `cd` fails, the script halts instantly, preventing the destructive `rm` command from ever running.

#### `set -u` (nounset)
By default, if you reference a variable that you haven't defined, Bash doesn't complain. It just substitutes it with an empty string.

`set -u` changes this. It tells Bash: "Treat unset variables as a critical error and exit immediately."

**The Danger Without It:**
```bash
#!/bin/bash
# Suppose $TARGET_DIR was supposed to be passed as an argument, but the user forgot.
TARGET_DIR=$1

rm -rf "$TARGET_DIR/*"
```
*If the user forgets to pass an argument, `$TARGET_DIR` becomes blank. The command expands to `rm -rf /*`, which will attempt to wipe out your entire operating system.*

**With `set -u`:**
Bash will stop at the `rm` line and throw an error: `line 5: TARGET_DIR: unbound variable`, saving your system.

#### `set -o pipefail`
This fixes a major loophole in how Bash handles pipelines (commands chained together with `|`).

Normally, the exit status of a pipeline is determined only by the very last command. If the first command fails but the second one succeeds, Bash thinks the whole pipe was a success.

`set -o pipefail` changes this. It tells Bash: "If any command in a pipeline fails, the entire pipeline is considered a failure."
...
# Attempt to grep a file that does not exist
grep "important_setting" non_existent_file.txt | awk '{print $2}'

echo "Script kept running!"
```
*Even with `set -e` enabled, this script will not exit. Why? Because `grep` fails (returns `1`), but its empty output is passed to `awk`. `awk` successfully processes the empty input and returns `0`. Because `awk` was the last command, Bash considers the line a success and keeps going.*

**With `set -o pipefail`:**
Bash looks at the whole chain. It sees `grep` failed, marks the entire pipeline as a failure, and `set -e` kicks in to abort the script.

### How to Survive "Strict Mode" (Edge Cases)
When you turn on strict mode, Bash becomes highly sensitive. You will need to know how to handle situations where you expect something to fail or be empty.

**1. How to allow a specific command to fail**
If a command might fail but you don't want the script to die (e.g., checking if a directory exists), use `|| true` or `|| :`.
```bash
# If the directory doesn't exist, mkdir fails, but '|| true' forces a success status
mkdir /dangerous/path || true
```

**2. How to handle optional/empty variables**
Because `-u` bans undefined variables, you must provide a fallback default value for variables that might be blank using the `${VAR:-default}` syntax.
```bash
# If $1 is empty, TARGET_DIR defaults to '/tmp' instead of throwing an unbound variable error
TARGET_DIR=${1:-/tmp}
```

**3. Masking errors in if statements**
You don't need to worry about `if` statements or loops. `set -e` is smart enough to know that a failed command inside an `if` condition is just a test, so it won't crash your script.
```bash
# This will NOT crash the script, even if grep finds nothing
...
- **Quote Everything:** Always use double quotes around variables (e.g., `"$var"`) to prevent word-splitting and globbing bugs caused by spaces or special characters.
- **Use Modern Conditionals:** Prefer `[[ ... ]]` over `[ ... ]` or `test` for better performance and safety, as it handles empty variables more gracefully and supports regex.
- **Cleanup with Traps:** Use the `trap` command to ensure temporary files or background processes are cleaned up when the script exits or is interrupted.
